...
my @filenames = glob(' /projects/data/seeker_results_FINAL/SRR* '); #create an array where each element is the name of one of the seeker result files
...
for (my $i = 0; $i < $number_of_files; $i++)
{
    ...
    $sample =~ s/\/projects\/data\/seeker_results_FINAL\///g;
...
my $a = 'mkdir /projects/data/SCRIPT_TEST/Reference_DB';
system($a);

my $b = 'mkdir /projects/data/SCRIPT_TEST/Reference_DB/greater_than_80';
system($b);

my $e = 'mkdir /projects/data/SCRIPT_TEST/Reference_DB/less_than_80';
system($e);

foreach my $key (keys %related_species_gt80)
{
    my $output = "/projects/data/SCRIPT_TEST/Reference_DB/greater_than_80/".$key.".txt";
...
{
    my $output = "/projects/data/SCRIPT_TEST/Reference_DB/less_than_80/".$key.".txt";
    open(OUT, "+>", $output); #open the output file
    print OUT "@{$related_species_lt80{$key}}\n"; #print list of samples
}

my $w = 'python RefMover.py';
system($w);
